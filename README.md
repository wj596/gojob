...
轻量级的任务调度解决方案，基于Go语言实现，其协程并发机制可支持大规模任务并行调度。gojob为调度节点，你的业务系统就是执行节点，凡是暴露出HTTP服务的业务方法都可以被调度，对业务系统完全无侵入。HTTP协议夸平台，契合当前主流的微服务架构。界面化的作业元数据管理，可以方便的将散落在各个业务系统的定时任务进行收拢，由gojob统一管理和调度。

通过Raft共识算法(强数据一致性协议) 、数据库异步多写、执行节点路由等措施。消除调度节点、执行节点和数据库节点的单点隐患，使gojob调度系统具有极高的可用性，无人值守亦可高枕无忧。

gojob使用方便，一般情况下，下载解压安装包，配置好MySQL连接(表结构会自动创建)属性，即可运行。



# 功能特性

- 极少依赖： 只依赖MySQL 数据库，分布式环境下使用内建的分布式协调机制，不需要安装第三方分布式协调服务，如Zookeeper、Etcd等；更少的依赖意味着需要更少的运维成本。
