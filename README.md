...
轻量级分布式任务调度解决方案，Go语言实现，其协程并发机制可支持大规模任务并行调度。gojob使用方便，一般情况下，下载解压安装包，配置好MySQL连接(表结构会自动创建)属性，即可运行。

gojob为调度节点，你的业务系统就是执行节点，凡是暴露出HTTP服务的业务方法都可以被调度，对业务系统完全无侵入。HTTP协议平台，契合当前主流的微服务架构。界面化的作业元数据管理，可以方便的将散落在各个业务系统的定时任务进行收拢，由gojob统一管理和调度。
